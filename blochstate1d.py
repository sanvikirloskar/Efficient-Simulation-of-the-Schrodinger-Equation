...
class GroundBlochState():
    ...
    def H_builder(self, q=0):
        main_diag = np.zeros(self.c.dims)
        for i in range(-self.c.N, self.c.N + 1):
            main_diag[i + self.c.N] = (((2*i + q) / (self.c.hbar * self.c.kl))**2 * self.c.E_r)
    ...
    def generate_bloch_state(self, spacial_dims=1, band=0, q=0): # if you don't specify the dimensions it will automatically be 1d, but 2d can be called explicitly
        ...
        psi1d = np.exp(1j * q * self.x_grid) * u
