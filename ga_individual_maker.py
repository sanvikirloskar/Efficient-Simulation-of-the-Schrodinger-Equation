...
class OpticalLatticeHamiltonian(HamiltonianSeparable):
    ...
    def pe_diagonals(self, controls: Optional[Controls] = None) -> RVectorSeq:
        ...
        """
        pe_diag: RVectorSeq = (-0.5* self.domain.constants.v0 * np.cos((2 * self.domain.constants.kl * self.domain.position_axes[0]) + controls)), (-0.5* self.domain.constants.v0 * np.cos((2 * self.domain.constants.kl * self.domain.position_axes[1]) + controls))
...
def make_controls_fn1(T,A_coeffs, B_coeffs, omegas):
    ...
    NOTE: Improved version of this is in comments
    """
    ...
    #def controls_fn(time: float) -> Controls:
      ...
      ##  N_elements = OLConstants.N_elements
       # for k in range(N_elements// 2):
          #  function += A_coeffs[k] * np.cos(omegas[k] * time) #selects the first half of the coefficients for the cosine terms
          #  function += B_coeffs[k] * np.sin(omegas[k] * time) #selects the second half of the coefficients for the sine terms
       # envelope =  (np.sin( (np.pi * time) / (T)))**2
      #  phi = (1 + function) * envelope
      #  return phi
