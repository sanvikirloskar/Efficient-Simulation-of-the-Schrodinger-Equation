...
import numpy as np
from QuEvolutio.quevolutio.core.aliases import (  #
    GVector,
    RVectorSeq,
    CTensors,
    CSRMatrix,
...
from crab_propagation_tools import make_total_base_pulse
...
def make_controls_fn(N_basis, A_coeffs, B_coeffs, omegas, T, base_pulse):
    def controls_fn(time: float) -> Controls:
        ...
        phi = (base_pulse(time) + crab_function) * envelope
        return phi 
    return controls_fn
N_basis = 10
shaking_func = make_controls_fn(N_basis, A_coeffs, B_coeffs, omegas, 10.0, base_pulse)
