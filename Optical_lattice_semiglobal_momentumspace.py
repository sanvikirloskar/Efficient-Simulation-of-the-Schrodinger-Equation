...
class OpticalLatticeHamiltonian(Hamiltonian):
    ...
    def __init__(
        ...
        self.l_vals = np.arange(-self.domain.constants.N, self.domain.constants.N + 1)
        # Pre-compute the kinetic energy diagonal.
        self._ke_diagonal: RVector = (self.domain.constants.hbar * (self.domain.constants.q + 2 * self.l_vals * self.domain.constants.kl)) ** 2 / (
            2.0 * self.domain.constants.mass
        )
        # Pre-compute the lattice potential energy diagonal.
        self._pe_diagonal: RVector = ( self.domain.constants.v0
        )
    ...
    def ke_action(self, state: GVector, controls: Optional[Controls] = None) -> GVector:
        ...
        return self._ke_diagonal * state
    ...
    def pe_action(self, state: GVector, controls: Optional[Controls] = None) -> GVector:
        ...
        out[1:] += -self._pe_diagonal / 4 * state[:-1]
        out[:-1] += -self._pe_diagonal / 4 * state[1:]
        return out
...
domain: QuantumHilbertSpace = QuantumHilbertSpace(
        ...
        position_bounds=np.array([[-6 * DHOConstants.unit_cell , 6 *DHOConstants.unit_cell]]),
        constants=constants,
    )
...
filename: str = "Optical_Lattice_SemiGlobal_momentumspace"
