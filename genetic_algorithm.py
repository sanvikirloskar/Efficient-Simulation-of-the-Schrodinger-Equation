...
def elitism(fitnesses, A_coeffs, B_coeffs, omegas, number_dead):
    """
...
def full_genetic_algorithm(A_coeffs, B_coeffs, omegas, mom_pop_des, number_dead):
    ...
    convergence = 0.115 #if the best fitness is less than this, we can stop the algorithm
    print('Termination value', convergence)
    best_fit_each_gen = []
    for i in range(num_iter):
        fitnesses, mom_pop = fitness_finder(A_coeffs, B_coeffs, omegas, mom_pop_des)
        best_A, best_B, best_omegas, remaining_A, remaining_B, remaining_omegas, best_fitness = elitism(fitnesses, A_coeffs, B_coeffs, omegas, number_dead)
